...
with open('task_1/tickers.txt', 'r') as file:
    for line in file:
        ...
        for index, row in ticker_df.iterrows():
            previous_index = index - 5
            # only can calculate with past 5 days data if data is available
            if previous_index >= 0:
                close_five_days_prior = ticker_df.iloc[previous_index, ticker_df.columns.get_loc('Close')]
                close_prev_day = ticker_df.iloc[index-1, ticker_df.columns.get_loc('Close')]
                price_momentum = (close_prev_day - close_five_days_prior)/(close_five_days_prior)*100
                ticker_column.append(price_momentum)
            else:
                ticker_column.append("")
        # full factor data from ticker inserted into dictionary
        price_momentum_data[ticker_symbol] = ticker_column
        
    price_momentum_df = pd.DataFrame(price_momentum_data)
    
    # adding date column as index
    microsoft_df = pd.read_csv(f'task_1/data/MSFT.csv')
    price_momentum_df = price_momentum_df.assign(Date=microsoft_df['Date'])
    price_momentum_df.set_index('Date', inplace=True)
    
    # outputting dataframe to csv
    price_momentum_df.to_csv('task_2/price_momentum_factor.csv')
